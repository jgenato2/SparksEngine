...
bool computeImportedModelWorldBounds(const render::ImportedModelData& model, glm::vec3& outMin, glm::vec3& outMax) {
    if (model.vertices.size() < 3) {
        return false;
    }

    glm::mat4 transform(1.0f);
    transform = glm::translate(transform, model.position);
    const glm::vec3 rot = glm::radians(model.rotationEulerDegrees);
    ...
    const std::size_t stride = (model.vertices.size() % 8 == 0) ? 8u : 3u;
    for (std::size_t i = 0; i + 2 < model.vertices.size(); i += stride) {
        const glm::vec3 localPos(model.vertices[i], model.vertices[i + 1], model.vertices[i + 2]);
        const glm::vec3 worldPos = glm::vec3(transform * glm::vec4(localPos, 1.0f));
...
void drawSceneTab(
    ...
    if (false && ImGui::CollapsingHeader("FBX Animation Import Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (importedModelSelected && importedModel.has_value()) {
        glm::vec3 importedMin(0.0f);
        glm::vec3 importedMax(0.0f);
        if (computeImportedModelWorldBounds(*importedModel, importedMin, importedMax)) {
            selectedCenter += (importedMin + importedMax) * 0.5f;
            boundMin = glm::min(boundMin, importedMin);
            boundMax = glm::max(boundMax, importedMax);
        } else {
    ...
    if (transformDragging && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
        ...
        constexpr float kScaleDragSensitivity = 0.0015f;
        ...
        if (importedModelSelected && importedModel.has_value() && transformDragStartImportedModel.has_value()) {
            ...
            if (transformMode == TransformMode::Move) {
                ...
                glm::vec3 startBoundsMin(0.0f);
                glm::vec3 startBoundsMax(0.0f);
                const bool hasStartBounds = computeImportedModelWorldBounds(startModel, startBoundsMin, startBoundsMax);
                const glm::vec3 startBoundsCenter = hasStartBounds
                    ? (startBoundsMin + startBoundsMax) * 0.5f
                    : transformDragStartSelectedCenter;

                const float scaleFactor = std::exp(dragAmount * kScaleDragSensitivity);
                modelData.scale = startModel.scale;
                const float minScaleX = std::max(0.05f, std::abs(startModel.scale.x) * 0.12f);
                const float minScaleY = std::max(0.05f, std::abs(startModel.scale.y) * 0.12f);
                const float minScaleZ = std::max(0.05f, std::abs(startModel.scale.z) * 0.12f);
                if (activeTransformAxis == TransformAxis::X) {
                    const float absScaled = std::clamp(std::abs(startModel.scale.x) * scaleFactor, minScaleX, 1000.0f);
                    modelData.scale.x = std::copysign(absScaled, startModel.scale.x);
                } else if (activeTransformAxis == TransformAxis::Y) {
                    const float absScaled = std::clamp(std::abs(startModel.scale.y) * scaleFactor, minScaleY, 1000.0f);
                    modelData.scale.y = std::copysign(absScaled, startModel.scale.y);
                } else if (activeTransformAxis == TransformAxis::Z) {
                    const float absScaled = std::clamp(std::abs(startModel.scale.z) * scaleFactor, minScaleZ, 1000.0f);
                    modelData.scale.z = std::copysign(absScaled, startModel.scale.z);
                }

                glm::vec3 scaledBoundsMin(0.0f);
                glm::vec3 scaledBoundsMax(0.0f);
                if (computeImportedModelWorldBounds(modelData, scaledBoundsMin, scaledBoundsMax)) {
                    const glm::vec3 scaledBoundsCenter = (scaledBoundsMin + scaledBoundsMax) * 0.5f;
                    modelData.position += startBoundsCenter - scaledBoundsCenter;
                }
            }
            ...
            for (int i = 0; i < static_cast<int>(selectedCloudIndices.size()) && i < static_cast<int>(transformDragStartClouds.size()); ++i) {
                ...
                if (transformMode == TransformMode::Move) {
                    ...
                    const float minScaleX = std::max(0.05f, std::abs(startCloud.scale.x) * 0.12f);
                    const float minScaleY = std::max(0.05f, std::abs(startCloud.scale.y) * 0.12f);
                    const float minScaleZ = std::max(0.05f, std::abs(startCloud.scale.z) * 0.12f);
                    if (activeTransformAxis == TransformAxis::X) {
                        const float absScaled = std::clamp(std::abs(startCloud.scale.x) * scaleFactor, minScaleX, 1000.0f);
                        cloud.scale.x = std::copysign(absScaled, startCloud.scale.x);
                    } else if (activeTransformAxis == TransformAxis::Y) {
                        const float absScaled = std::clamp(std::abs(startCloud.scale.y) * scaleFactor, minScaleY, 1000.0f);
                        cloud.scale.y = std::copysign(absScaled, startCloud.scale.y);
                    } else if (activeTransformAxis == TransformAxis::Z) {
                        const float absScaled = std::clamp(std::abs(startCloud.scale.z) * scaleFactor, minScaleZ, 1000.0f);
                        cloud.scale.z = std::copysign(absScaled, startCloud.scale.z);
                    }
                }
            }
    ...
    const ImVec2 axisCenter(imgMax.x - 30.0f, imgMin.y + 30.0f);
...
void drawRightPane(
    ...
    if (importedModelSelected && importedModel.has_value()) {
        ...
        // Multi-cloud selection â€” use first selected cloud as shared reference
        int firstValid = -1;
        for (const int idx : selectedCloudIndices) {
            if (idx >= 0 && idx < static_cast<int>(environmentSettings.cloudObjects.size())) {
                firstValid = idx;

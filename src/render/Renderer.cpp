...
unsigned int createSkydomeProgram() {
    ...
    static constexpr const char* kFragmentShader = R"(#version 460 core
        ...
        void main() {
            ...
            float minY = 0.18; // Lower clamp for more horizon fade
            float yGuard = max(skyDir.y, minY);
            // Stretch clouds near horizon for realism
            float horizonStretch = mix(2.2, 1.0, smoothstep(minY + 0.04, 0.55, skyDir.y));
            vec2  cloudPlane = skyDir.xz / yGuard * horizonStretch;

            // Soften and widen fade near the bottom of the dome (low y)
            float bottomFade = smoothstep(minY, minY + 0.22, skyDir.y);
            ...
            float horizFade = smoothstep(0.0, 0.18, skyDir.y); // wider, softer fade
            ...
            // Atmospheric perspective: blue-tint and desaturate clouds near horizon
            float horizonT = 1.0 - smoothstep(0.0, 0.22, skyDir.y);
            vec3 horizonCloudTint = mix(cloudLit, mix(uHorizonColor, vec3(0.68, 0.78, 0.98), 0.42), horizonT * 0.55);
            color = mix(color, horizonCloudTint, cumAlpha * cloudAmt);

...
unsigned int createTerrainProgram() {
    ...
    static constexpr const char* kFragmentShader = R"(
        ...
        uniform float uEnvBloom;
        uniform vec3 uLightDir; // Sun direction for shadow
        ...
        void main() {
            ...
            // --- Simple sun-cast shadow: darken terrain facing away from sun ---
            float shadow = smoothstep(-0.15, 0.15, dot(n, uLightDir));
            shadow = mix(0.45, 1.0, shadow); // 0.45 = shadow darkness
            vec3 color = base.rgb * lit * shadow + uEmissive + uSunColor * sunGlow;
            // Add environmental bloom from sky (sun-dependent atmospheric lighting)
            color += uSunColor * uEnvBloom * 0.5;
